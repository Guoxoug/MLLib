...
from MLLib.utils.utils import PerturbInput
...
def run_epoch_perturb(loader, model, perturbation, onehot_criterion, soft_criterion, optimizer, device_type='cuda', epoch=0, n_epochs=0, train=True, class_num=100, perturb_degree=5, sample_num=20, alpha=0.5, pt_input_ratio=0.25, log_every_step=True):
    time_meter = Meter(name='Time', cum=True)
    loss_meter = Meter(name='Loss', cum=False)
    error_meter = Meter(name='Error', cum=False)
    ...
    for i, (input, target) in enumerate(loader):
        if train:
            ...
            p_n = torch.norm(pt_flat, p=2, dim=1, keepdim=True).unsqueeze(1).unsqueeze(1).expand_as(pt_random)
            pt = pt_random.div_(p_n)
            pt_= pt.unsqueeze(0).expand(pt_batch, sample_num, input_size[1], input_size[2], input_size[3])
            pt_input = input[:pt_batch].unsqueeze(1).expand_as(pt_) + pt_
            pt_input =torch.reshape(pt_input, (pt_batch*sample_num, input_size[1], input_size[2], input_size[3])) 
            input = move_to_device(input, device_type, False)
            target = move_to_device(target, device_type, False)
            ...
            pt_output_sum = torch.sum(torch.eq(p_outputs, pt_target.expand_as(p_outputs)).float(), dim=1, keepdim=True)
            '''
            ...
            loss = alpha * onehot_loss + (1-alpha)*pt_input_ratio * perturb_loss
...
def train(dataset, net_arch, trn_para, save, device_type='cuda', model_filename='model.pth'):
    ...
    perturbation = move_to_device(PerturbInput((trn_para['batch_size'], 3, 32, 32),perturb_degree=trn_para['perturb_degree'], sample_num=trn_para['sample_num'], pt_input_ratio=trn_para['pt_input_ratio'], num_classes=dataset.num_classes), device_type)
...
if __name__ == '__main__':
    ...
    if not os.path.isdir('CMDs'):
        os.mkdir('CMDs')
    with open('CMDs/step_train_pert_cal_densenet.cmd', 'a') as f:
